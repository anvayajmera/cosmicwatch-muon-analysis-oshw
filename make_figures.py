...
def main() -> None:
    ...
    corr.to_csv(OUT_DIR / "correlation_table.csv", index=False)

    session_table = session_barometric_table(df)
    session_table.to_csv(OUT_DIR / "session_summary.csv", index=False)

    coef_table = model_coefficients_table(models)
    coef_table.to_csv(OUT_DIR / "model_coefficients.csv", index=False)

    resid_lb = acorr_ljungbox(m_diurnal.resid, lags=[1, 6, 12], return_df=True)
    resid_lb = resid_lb.rename_axis("lag").reset_index()
    resid_lb.to_csv(OUT_DIR / "residual_ljungbox.csv", index=False)
    ...
    with open(OUT_DIR / "paper_analysis_summary.txt", "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            f"(max {fig_metrics['max_hourly_sem_pct']:.4f} %), which is above the fitted diurnal amplitude.\n\n"
        )
    ...
    print("Done.")
    print(f"Figures and tables saved to: {OUT_DIR.resolve()}")
